...
$Store.on(DataInit.doneData, (_, repos) => {
    return {
        ...repos,
        main_questions: repos?.main_questions?.sort((a, b) => a.questions.length > b.questions.length ? 1 : -1)
    }
})
$SearchArray.on(DataInit.doneData, (_, { main_answers, main_questions }) => {
    const qu_an = main_answers?.map(el => {
        return {
            answer: el?.answers,
            question: main_questions?.find(element => el.question_id === element.question_id)?.questions
        }
    })
    return qu_an
})
